//prepmee_website\src\context\AuthContext.js
'use client';

import React, { createContext, useState, useContext, useEffect, useCallback } from 'react';
import { authService } from '@/services/auth.service_backup';
...
export const AuthProvider = ({ children }) => {
  ...
  const registerTutor = async (userData) => {
    ...
    try {
      ...
      // After registration, redirect to login page instead of verify-email
      const locale = window.location.pathname.split('/')[1];
      router.push(`/${locale}/tutor_login?status=success&message=${encodeURIComponent('Application submitted! Please log in.')}`);
      return data;
    } catch (error) {
      setError(error.message);
      throw error;
